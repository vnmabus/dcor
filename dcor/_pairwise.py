...
def rowwise(function, x, y, **kwargs):
    """
    rowwise(function, x, y=None, *, map=None, is_symmetric=None, **kwargs)
...
def _rowwise_imp(function, x, y, map_function=map, force_naive=False,
                 **kwargs):
    """
    Real implementation of :func:`rowise`.
    ...
    if rowwise_function and not force_naive:
        result = rowwise_function(x, y, map_function=map_function, **kwargs)
        if result is not NotImplemented:
            return result

    map_aux_func = functools.partial(_rowwise_map_aux_func, function=function)

    return np.array(list(map_function(map_aux_func, zip(x, y))))
